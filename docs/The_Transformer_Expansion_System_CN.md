...
关键地，我们揭示了多头注意力（MHA）表现出强烈的温度响应性，而多查询注意力（MQA）由于其各向同性几何性质，表现出接近零的调制能力（C ≈ 0）。这些现象在我们提出的三原理框架下无缝统一：**注意力决定混合模式，嵌入决定可观测模式，Logits 反映过滤后的动力学**。此外，我们展示了表征演化（稳定性指数 SI）经历平滑的连续过渡（LP → SP → OSC），而非离散的相变，这将深度语言模型与传统热力学系统区分开来。
