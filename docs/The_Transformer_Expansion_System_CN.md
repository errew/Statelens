...
我们提出了一个全面的经验框架，用于理解基于 Transformer 的大语言模型（LLMs）的内部动力学。通过对 20 多个模型跨多种架构的系统分析，我们建立了深度表征的三个基本物理定律：（1）Transformer 层作为**扩张动力学系统**运行，其特征是普适谱半径 **λ ≈ 1.88 > 1**，从根本上挑战了收缩表征学习的传统假设；（2）MLP 权重主方向与激活空间主成分之间的几何对齐遵循严格的 **K-θ 单调性定律**，为流形演化提供了统一的几何机制；（3）注意力温度 β 通过温度依赖方程 **τ = τ_min + C/β** 调制混合时间 τ，该方程严格受底层注意力架构约束。

关键地，我们揭示了多头注意力（MHA）表现出强烈的温度响应性，而多查询注意力（MQA）由于其各向同性几何性质，表现出接近零的调制能力（C ≈ 0）。这些现象在我们提出的三公理框架下无缝统一：**注意力决定混合模式，嵌入决定可观测模式，Logits 反映过滤后的动力学**。此外，我们证明了表征演化（稳定性指数 SI）经历平滑的连续过渡（LP → SP → OSC），而非离散的相变，这将深度语言模型与传统热力学系统区分开来。
...
1. **Transformer 是扩张系统**：层严格在 λ ≈ 1.88 > 1 的区域内运行。
...
**发现**：Transformer 层的 Jacobian 谱半径在多个系列中一致评估为 **λ ≈ 1.88 > 1**。

| 模型 | 架构 | λ（谱半径） | 动力学区域 |
|:-----|:-----|:------------|:-----------|
| Qwen2.5-0.5B | GQA | 1.87 | 扩张 |
| Qwen2.5-1.5B | GQA | 1.89 | 扩张 |
| Qwen2.5-3B | GQA | 1.88 | 扩张 |
| phi-2 | MHA | 1.91 | 扩张 |
| gemma-2b | MQA | 1.86 | 扩张 |
...
**图 1：多模型 Jacobian 谱分析**
![Multi-Model Jacobian Spectrum](../validation_results/multi_model_full_block_jacobian.png)
*图 1 展示了 20+ 个模型的 Jacobian 谱半径分布，所有模型均显示 λ ≈ 1.88 > 1，验证了扩张动力学假设。*
...
**图 2：K-θ 单调性验证**
![K-θ Validation](../validation_results/k_star_validation.png)
*图 2 展示了 K-θ 单调性定律的验证结果，角度 θ_k 随主维度 K 严格单调递减，符合指数饱和曲线。*
...
**图 3：跨架构温度分析**
![Cross-Architecture Temperature Analysis](../validation_results/cross_architecture_temperature_analysis.png)
*图 3 展示了 MHA、GQA 和 MQA 架构的温度调制能力差异，MQA 表现出接近零的响应性（C ≈ 0）。*
...
**图 4：熵与温度关系**
![Entropy vs Temperature](../validation_results/entropy_vs_temperature.png)
*图 4 展示了表征熵与温度的关系，揭示了连续过渡特征而非离散相变。*
...
**结论**：对齐可以在保持功能的同时被破坏，证明**对齐不是功能副产物**，而是独立的因果结构变量。
...
**图 5：谱能量比较**
![Spectral Energy Comparison](../validation_results/spectral_energy_comparison.png)
*图 5 展示了不同层的谱能量分布比较，验证了扩张-归一化动力学机制。*
...
1. **信息增长（拉伸）**：线性和注意力变换施加谱半径 λ ≈ 1.88，拉伸语义流形。
...
- **修正收缩假设**：基于 λ < 1 的理论必须更新以考虑 λ ≈ 1.88 的经验现实。
...
- **代码与数据**：所有 100+ 个原始 JSON 实验输出和 60+ 个诊断脚本在本仓库中开源。
