...
| 模型 | 架构 | λ（谱半径） | 动力学区域 |
|:-----|:-----|:------------|:-----------|
| Qwen2.5-0.5B | GQA | 1.72 | 扩张 |

**方法论说明**：谱半径测量针对 Layer 2（语义稳定区）的最后一个 token 进行。Layer 2 代表了模型处理完初始嵌入后的稳定语义处理阶段，避免了浅层初始化效应和深层特化效应。最后一个 token 的选择符合自回归生成的因果性要求，确保测量的局部动力学独立性。
...
**图 1：Layer 2 Jacobian 谱分析**
![Layer 2 谱半径](../../validation_results/figures/layer2_spectral_radius_analysis.png)
*图 1 展示了 7 个模型在 3 种输入分布下的 Layer 2 Jacobian 谱半径。所有模型显示 λ > 1（扩张），紫色虚线表示论文参考值 λ ≈ 1.88。Qwen2.5-0.5B Normal 对话达到 λ = 1.72，与理论预测一致。*

**图 2：架构对比**
![架构对比](../../validation_results/figures/architecture_comparison.png)
*图 2 展示了按注意力架构类型的平均谱半径。GQA（分组查询注意力）显示最高平均 λ ≈ 1.97，而 MHA（多头注意力）显示最稳定的 λ ≈ 1.55。误差条表示跨模型和输入类型的标准差。*
