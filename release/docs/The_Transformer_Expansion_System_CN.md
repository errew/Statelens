...
**发现**：Transformer 层的 Jacobian 谱半径评估为 **λ ≈ 1.88 > 1**。

| 模型 | 架构 | λ（谱半径） | 动力学区域 |
|:-----|:-----|:------------|:-----------|
| Qwen2.5-0.5B | GQA | 1.88 | 扩张 |
...
*图 1 展示了多个模型的 Jacobian 谱半径分布，所有模型均显示 λ > 1，验证了扩张动力学假设。*
...
**图 6：对齐崩塌与行为保持（TEST_008）**
![Alignment Collapse TEST_008](../validation_results/figures/alignment_collapse_test008.png)
*图 6 展示了逐层对齐崩塌实验。主角度从 66.69° 增加到 85.76°（接近正交），而功能变化保持在 <0.4%，证明几何对齐与模型功能解耦。*
...
1. **信息增长（拉伸）**：线性和注意力变换施加谱半径 λ ≈ 1.7-1.8 (Layer 2)，拉伸语义流形。
